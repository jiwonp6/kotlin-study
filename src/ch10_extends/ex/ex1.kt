...
class Bird(name: String): Animal(name), FlyingAnimal {
    override fun eat() {
        println("${name}(이/가) 씨앗을 먹습니다.")
    }
}

class Fish(name: String): Animal(name), SwimmingAnimal {
    override fun eat() {
        println("${name}(이/가) 플랑크톤을 먹습니다.")
    }
}

class Duck(name: String): Animal(name), FlyingAnimal, SwimmingAnimal {
    override fun eat() {
        println("${name}(이/가) 풀속에 있는 벌레를 먹습니다.")
    }

    override fun fly() {
        println("${name}(이/가) 물에서 날아오릅니다.")
    }

    override fun swim() {
        println("${name}(이/가) 연못을 헤엄칩니다.")
    }
}
