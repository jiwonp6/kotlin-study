...
class Outer {
    ...
    // 키워드 선언 없이 사용 (클래스 안에 클래스를 만들 때의 권장사항)
    // Java 에서는 static 키워드를 붙인 경우에 해당
    class Nested {
        fun function() = 2
        // 중첩 클래스에서는 바깥 클래스의 멤버에 접근 할 수 없음
        // val nestedNumber = number + 1
    }

    // 내부 클래스 (비권장사항)
    // 내부 클래스가 외부 클래스의 정보를 가지고 있어서 메모리 누수 위험
    // 코틀린에서는 inner 키워드 사용하지 않으면 모두 중첩 클래스
    inner class Inner {
        // 바깥 클래스를 참조하는 내부 클래스에
        // 같은 이름의 프로퍼티나 메소드가 있을 경우
        // 이름의 혼란이 생김
        ...
        fun ref() {     // 같은 이름의 멤버 참조
            ...
            println("내부 클래스의 number: ${this.number}")
        }
    }
}
...
fun main() {
    // 중첩 클래스
    // Outer 클래스의 인스턴스 생성 없이 중첩 클래스에 접근하고 생성 가능
    val nested = Outer.Nested()
    println(nested.function())
    
    // 내부 클래스
    // 바깥 클래스의 인스턴스 생성 없이 생성 불가
